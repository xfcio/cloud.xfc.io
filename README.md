...
gcloud compute instances create "master1" --zone "asia-east1-a" --machine-type n1-standard-1 \
  --image "centos-7-v20170719" --image-project "centos-cloud" --boot-disk-size "20" \
...
./gk-deploy -g -t ./ocp-templates -n cns -c oc --admin-key adminpass --user-key userpass  -v topology.json
>  --load if restarting
```
